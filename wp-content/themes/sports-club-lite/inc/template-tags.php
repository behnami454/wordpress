...
if ( ! function_exists( 'sports_club_lite_the_attached_image' ) ) : 
/**
 * Prints the attached image with a link to the next attached image.
 */
function sports_club_lite_the_attached_image() {
	$post                = get_post();
	$attachment_size     = apply_filters( 'sports_club_lite_attachment_size', array( 1200, 1200 ) );
...
function sports_club_lite_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		...
		// This blog has only 1 category so sports_club_lite_categorized_blog should return false
		return false;
	}
}

/**
 * Flush out the transients used in sports_club_lite_categorized_blog
 */
